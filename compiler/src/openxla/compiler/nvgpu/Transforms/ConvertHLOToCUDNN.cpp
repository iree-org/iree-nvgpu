...
class ConvertHLOToCUDNNPass
 ...
 public:
  void runOnOperation() override {
    // Outline relevant clusters into CUDNN graph.
    MLIRContext* ctx = &getContext();
    RewritePatternSet outliningPatterns(ctx);
    auto cudnnHandleCache = std::make_shared<CudnnHandleCache>();
    outliningPatterns.add<OutlineCudnnGraphPattern<stablehlo::ClampOp>>(
        ctx, cudnnHandleCache);
    outliningPatterns.add<OutlineCudnnGraphPattern<stablehlo::ConvolutionOp>>(
        ctx, cudnnHandleCache);
    if (failed(applyPatternsAndFoldGreedily(getOperation(),
                                            std::move(outliningPatterns)))) {
      return signalPassFailure();
    }

    // Convert CUDNN graph to CUDNN ops.
    RewritePatternSet conversionPatterns(ctx);
    conversionPatterns.add(&convertClamp);
    conversionPatterns.add(&convertConv);
    if (failed(applyPatternsAndFoldGreedily(getOperation(),
